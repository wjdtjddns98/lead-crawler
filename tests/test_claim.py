...
def test_claim_tops_up_to_target(settings) -> None:
    ...
    with session_scope(settings) as s:
        items = claim_work(s, a, batch=15, cap=15, now=_T0)
    assert len(items) == 15
...
def test_two_users_get_disjoint_sets(settings) -> None:
    ...
    with session_scope(settings) as s:
        ai = claim_work(s, a, batch=15, cap=15, now=_T0)
    with session_scope(settings) as s:
        bi = claim_work(s, b, batch=15, cap=15, now=_T0)
...
def test_idempotent_topup(settings) -> None:
    a = _uid(settings, "alice")
    with session_scope(settings) as s:
        first = {it["id"] for it in claim_work(s, a, batch=15, cap=15, now=_T0)}
    with session_scope(settings) as s:
        again = {it["id"] for it in claim_work(s, a, batch=15, cap=15, now=_T0)}
...
def test_refill_after_confirm(settings) -> None:
    ...
    with session_scope(settings) as s:
        ai2 = claim_work(s, a, batch=15, cap=15, now=_T0)
    assert len(ai2) == 15  # 확정으로 줄어든 만큼 새 항목 자동 리필.
    assert rid not in {it["id"] for it in ai2}  # 확정 항목은 작업분에서 빠짐.


def test_prefetch_additive_up_to_cap(settings) -> None:
    """"작업 받기" 1회 = +batch 추가(선취 허용), 총량은 cap 에서 멈춘다."""
    a = _uid(settings, "alice")
    with session_scope(settings) as s:
        assert len(claim_work(s, a, batch=8, cap=18, now=_T0)) == 8
    with session_scope(settings) as s:  # 남은 작업이 있어도 +8 선취.
        assert len(claim_work(s, a, batch=8, cap=18, now=_T0)) == 16
    with session_scope(settings) as s:  # cap(18)까지만 — +2.
        assert len(claim_work(s, a, batch=8, cap=18, now=_T0)) == 18
    with session_scope(settings) as s:  # cap 도달 — 신규 배정 0.
        assert len(claim_work(s, a, batch=8, cap=18, now=_T0)) == 18
    with session_scope(settings) as s:  # my_work 는 부작용 없는 조회.
        assert len(my_work(s, a)) == 18
        assert count_reviews(s) == _N - 18
...
def test_claims_are_permanent_no_ttl(settings) -> None:
    ...
    a, b = _uid(settings, "alice"), _uid(settings, "bob")
    with session_scope(settings) as s:
        claim_work(s, a, batch=20, cap=20, now=_T0)  # alice 전부 점유.
    with session_scope(settings) as s:  # 하루 뒤에도 bob 가져갈 것 없음.
        assert claim_work(s, b, batch=15, cap=15, now=_T0 + timedelta(days=1)) == []
...
def test_conflict_when_other_claims(settings) -> None:
    a, b = _uid(settings, "alice"), _uid(settings, "bob")
    with session_scope(settings) as s:
        rid = claim_work(s, a, batch=1, cap=1, now=_T0)[0]["id"]
...
def test_claimed_hidden_from_full_queue(settings) -> None:
    ...
    with session_scope(settings) as s:
        ...
        claim_work(s, a, batch=15, cap=15, now=_T0)
    with session_scope(settings) as s:
...
def test_claim_filter_by_country_alias(mixed_settings) -> None:
    ...
    with session_scope(mixed_settings) as s:
        items = claim_work(s, c, batch=10, cap=10, now=_T0, countries=["미국"])
...
def test_claim_filter_by_industry_case_insensitive(mixed_settings) -> None:
    ...
    c = _uid(mixed_settings, "carol")
    with session_scope(mixed_settings) as s:
        items = claim_work(s, c, batch=10, cap=10, now=_T0, industries=["finance"])
...
def test_claim_filter_by_listed_join(mixed_settings) -> None:
    ...
    c = _uid(mixed_settings, "carol")
    with session_scope(mixed_settings) as s:
        items = claim_work(s, c, batch=10, cap=10, now=_T0, listed="listed")
...
def test_claim_empty_filter_is_all(mixed_settings) -> None:
    ...
    c = _uid(mixed_settings, "carol")
    with session_scope(mixed_settings) as s:
        items = claim_work(s, c, batch=10, cap=10, now=_T0)
    assert len(items) == 5
...
def test_claim_filter_switch_keeps_claims(mixed_settings) -> None:
    ...
    c = _uid(mixed_settings, "carol")
    with session_scope(mixed_settings) as s:
        kr = claim_work(s, c, batch=10, cap=10, now=_T0, countries=["KR"])
    assert {it["country"] for it in kr} == {"KR"} and len(kr) == 2
    # 같은 직원이 US 로 범위 전환 → KR 점유 유지 + US 신규 배정(혼합 작업분).
    with session_scope(mixed_settings) as s:
        mine = claim_work(s, c, batch=10, cap=10, now=_T0, countries=["US"])
...
def test_cap_is_global_across_filters(mixed_settings) -> None:
    """총량 상한(cap)은 필터 무관 전역 — 세그먼트를 바꿔 선취해도 cap 을 못 넘는다."""
    c = _uid(mixed_settings, "carol")
    with session_scope(mixed_settings) as s:
        kr = claim_work(s, c, batch=3, cap=3, now=_T0, countries=["KR"])
    assert len(kr) == 2  # KR 은 2건뿐.
    with session_scope(mixed_settings) as s:  # US 로 전환해도 총량 3 상한 → 1건만 추가.
        mine = claim_work(s, c, batch=3, cap=3, now=_T0, countries=["US"])
    assert len(mine) == 3
...
def test_invalid_listed_raises_at_storage(mixed_settings) -> None:
    ...
    with session_scope(mixed_settings) as s:
        with pytest.raises(ValueError):
            claim_work(s, c, batch=10, cap=10, now=_T0, listed="LISTED")
    with session_scope(mixed_settings) as s:
        with pytest.raises(ValueError):
