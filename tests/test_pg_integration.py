...
def test_pg_concurrent_claim_same_filter_disjoint(pg_settings: Settings) -> None:
    ...
    try:
        ...
        ai = claim_work(sa, a, batch=4, cap=4, now=_T0, countries=["US"], listed="listed")
        bi = claim_work(sb, b, batch=4, cap=4, now=_T0, countries=["US"], listed="listed")
        sa.commit()
        sb.commit()
    finally:
        sa.close()
...
def test_pg_concurrent_claim_different_filters(pg_settings: Settings) -> None:
    ...
    sa, sb = sm(), sm()
    try:
        ai = claim_work(sa, a, batch=10, cap=10, now=_T0, countries=["KR"])
        bi = claim_work(sb, b, batch=10, cap=10, now=_T0, countries=["US"])
        sa.commit()
        sb.commit()
    finally:
        sa.close()
