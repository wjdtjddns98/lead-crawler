...
3. **배치 30개 + 총량 100개(선취 허용)**: 1회 "작업 받기" = **+30개 추가**(기존: 15까지 채우기).
   남은 작업이 있어도 다른 세그먼트 지시를 받아 **미리 받아둘 수 있고**(예: 13개 남은 상태에서
   +30 선취), 한 계정의 동시 점유 총량은 **100개 상한**(도달 시 신규 배정 0).
...
1. 직원이 "작업 받기" 클릭 → **+30개 추가 배정**된다(총량 100 상한 — 도달 시 추가 0).
   남은 작업이 있어도 새 지시(다른 세그먼트)를 받아 미리 받아둘 수 있다.
2. 받은 항목은 **처리 전까지 내 것** — 시간이 지나도, 로그아웃해도 안 뺏긴다. 재로그인하면
   `GET /queue/mine`(부작용 없는 조회)으로 그대로 복원.
3. 국가/업종/상장 **작업범위 필터는 신규 배정에만** 적용된다. 필터를 바꿔도 이미 받은 항목은
   반납되지 않고 내 작업분에 계속 남는다(총량 100 상한은 필터와 무관한 전역).
...
### 4.2 변경: `POST /queue/claim` — **추가형(additive)으로 의미 변경**
- 요청/응답 스키마는 **그대로**(`ClaimRequest{country,industry,listed}` → `list[ReviewItem]`).
- 의미 변경 (⚠️ 기존 "15까지 채우기(멱등)" → **"+30 추가"**):
  - 호출 1회 = 새 작업 **+30개** 추가 배정(선취). 총량 100 도달 시 신규 배정 0.
  - 응답에는 **필터와 무관하게 내 점유 전체**가 담긴다(필터는 신규 배정분에만 적용).
    → 필터를 바꾼 직후엔 이전 범위 항목 + 새 범위 항목이 섞여 내려올 수 있음(정상).
  - **추가형이므로 새로고침·복원 용도로 호출하면 안 된다** — 그 용도는 §4.3 `GET /queue/mine`.
    "작업 받기" 버튼 클릭 시에만 호출할 것.

### 4.3 신설: `GET /queue/mine` — 내 작업분 조회(부작용 없음)
- 응답: `list[ReviewItem]` (내가 점유 중인 pending 전체).
- 페이지 로드·새로고침·재로그인 복원·확정/거부 후 목록 갱신에 이걸 쓴다(추가 배정 없음).

### 4.4 변경: `GET /queue` (전체큐)
- 파라미터·응답 스키마 그대로. 단, **점유 중인 행은 items·total에서 제외**된다.
- 전체큐의 pending 건수 = "받아갈 수 있는 잔여 작업 수"로 의미가 명확해짐.

### 4.5 변경: confirm/reject 409 조건
- 타인이 점유한 항목은 시간 경과와 무관하게 **항상 409** (기존: TTL 지나면 처리 가능).
- 409 메시지: "다른 직원이 처리 중인 항목입니다."

### 4.6 신설: `POST /admin/users/{user_id}/reclaim` (관리자 전용)
```jsonc
// 요청 본문 없음. 응답:
...
### 4.7 변경: `GET /admin/users` 응답 필드 추가 (하위호환 — 필드 추가만)
```jsonc
...
- **BE-1 · TTL/반납 제거** (`storage/review.py`, `config.py`)
  ...
  - 설정: `review_claim_batch` 15→**30**(1회 추가량), `review_claim_cap`=**100**(총량 상한) 신설,
    `review_claim_ttl_minutes` 삭제. `claim_work` 는 추가형(+batch, cap 상한)으로 변경,
    부작용 없는 조회 `my_work` + `GET /queue/mine` 신설.
...
- **BE-5 · 테스트** (`tests/test_claim.py`, `test_pg_integration.py`)
  - 유지: 배타 배정·확정 후 리필.
  - 교체/신설: TTL 만료 재클레임 → **시간 경과해도 안 뺏김** / 반납 → **관리자 회수** /
    필터 전환 반납 → **필터 전환해도 점유 유지 + 전역 cap 상한** /
    **선취(+batch 반복, cap 에서 정지)·`my_work` 무부작용** / 전체큐 점유 숨김.
...
- **FE-2 · 조회/받기 분리 (⚠️ 중요)**: 페이지 로드·새로고침·재로그인 복원·확정/거부 후 목록
  갱신은 **`GET /queue/mine`**(부작용 없음)으로. `POST /queue/claim` 은 **"작업 받기" 버튼
  클릭 시에만** 호출(호출할 때마다 +30 추가되므로 자동 폴링/리필에 쓰면 안 됨).
  "반납" 계열 버튼 제거.
...
1. **방치 점유는 관리자 회수 전까지 잠김** — TTL 자동 복귀가 없으므로, 퇴사/부재 계정의 점유는
   관리자가 회수해야 풀린다(§4.6). 계정 비활성화가 점유를 자동 해제하지는 않는다(회수 별도).
2. **직원 스스로 손에서 뗄 방법은 거부뿐** — 애매한 항목도 확정/거부 중 하나로 종결해야 하고,
   선취를 포함해 총량 100을 넘으면 더 못 받는다. "받은 건 책임진다" 원칙의 의도된 결과.
...
2. `POST /queue/claim` 1회 = +30개 추가(선취 — 남은 작업이 있어도 추가 수령 가능),
   총량 100 도달 시 신규 배정 0(기존 점유만 반환). `GET /queue/mine` 은 몇 번을 불러도 점유 불변.
3. 필터를 KR→US로 바꿔도 KR 점유가 유지되고, 신규 배정은 US만 + 총량 100 상한.
