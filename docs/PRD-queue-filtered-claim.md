...
> 상태: **구현 완료 — 일부 폐기(Superseded)** · 작성일 2026-06-29 · 영역: 백엔드 + 프론트엔드 분리
> 관련 계약 변경: FE↔BE (엔드포인트 `GET /queue`, `POST /queue/claim` 시그니처 확장) — **양쪽 공유 필수**

> ⚠️ **2026-07-02 영구 배정 전환으로 이 문서의 일부가 폐기됨** — 정본은
> **`PRD-queue-claim-permanent.md`**. 폐기된 부분:
> - §3.4·BE-1 말미·FE-3 의 **"필터 전환 시 비매칭 점유 자동 반납(release)"** → 반납 자체가
>   삭제됨(점유는 처리 전까지 영구 귀속, 필터는 신규 배정에만 적용).
> - `release_non_matching`·`releaseWork()`·`POST /queue/release` → 전부 삭제됨.
> - claim 은 "target 까지 채우기(멱등)" → "+30 추가형(총량 100 상한)", 조회는 `GET /queue/mine`.
> - 검수 기준 §9-4(전환 시 반납) → 폐기. 국가/업종/상장 **필터링 자체**(§4~5 의 매칭 규약·
>   SKIP LOCKED 조인 잠금)는 여전히 유효.
