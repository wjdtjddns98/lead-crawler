...
- `run_pipeline(record_only=True)`: dedup·inline 도메인 해석 통과 후 원장 기록만 하고 pending(추출)에 넣지 않음. 발견 커서는 `discovery_cursor`(source, segment.label) 전역 커서 그대로(잡별 커서 없음). 발견 단계도 `_StallWatchdog` + `on_progress` beat.
