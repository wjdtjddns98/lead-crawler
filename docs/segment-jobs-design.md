...
6. **러너 퇴역** ✅ — `scripts/backfill_promote_domained.py`·테스트 삭제, 로컬 `run-promote-*`·`backfill_promote_import_oneoff.py` 제거(백업은 스크래치패드). import 시드 재패스는 같은 필터의 세그먼트 요청으로 대체.

## 7. 위험·함정
- Playwright 메모리: 워커 합 A2+C2+S3=7. 세대교체+정체 종료로 방어. 초과 시 S 워커 축소.
- 수동 스크립트 병행: PR6 에서 스크립트 삭제로 소멸(승격 경로 = 트랙 S 단일, S 락으로 동시 1건).
