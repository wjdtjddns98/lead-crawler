...
class KsicIndustryMapRow(Base):
    ...
    NPS 업종코드(6자리)가 1차 소비자. **주의: NPS 코드는 KSIC 10차가 아니라 구체계다**
    (2026-07-14 실측: 61xxxx=수상운송·21xxxx=제지·26xxxx=요업) — 10차 접두 역매핑
    (industry_from_ksic)을 여기 적용하면 전면 오라벨이 난다(546코드 실사고로 규칙
    패스 폐지). 코드가 ~1.6천 개뿐이라 코드 단위로 분류하면 스냅샷 수십만 행 전체가
    라벨링된다. ``method``: llm(닫힌 택소노미 분류기) 단독 — 'rule' 은 폐지된 구
    규칙 패스의 이력값. 라벨은 닫힌 집합(INDUSTRY_TAXONOMY+미분류) 밖 값을 절대
    담지 않는다(할루시네이션 게이트). 월간 재실행은 미등재 코드만 분류(멱등 —
    기존 매핑 재분류 0). 테이블명의 ksic_ 는 역사적 명명(위 코드계 주의 참조).
    """
