...
def _inc_slices(today: date | None = None) -> list[tuple[str | None, str | None]]:
    ...
    ponytail: 해가 바뀌면 새 연도가 **앞에** 끼어들어 저장된 slice_idx 가 한 칸씩 밀린다
    (1년 1회). 커버리지 **누락은 0** 이다 — 커서는 최신→과거로 단조 전진하므로 밀린 idx k
    가 새로 가리키는 연도는 이미 지난 사이클에 스캔한 연도이고, idx k+1 이 원래 다음 차례다.
    즉 비용은 "한 해를 한 번 재스캔"(세그먼트당 최대 ~100요청)뿐이라 방치한다. 정확히
    잡으려면 커서 키를 연도 문자열로 바꿔야 하는데 마이그레이션 비용이 이득보다 크다.
