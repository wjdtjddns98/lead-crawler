...
_BLOCKLIST = frozenset({
    ...
    # 글로벌 디렉터리·구인·기업DB·SNS·뉴스·평점(기업 직도메인 아님) — 검색 잡음 제거.
    "crunchbase.com", "f6s.com", "lusha.com", "zoominfo.com", "dnb.com",
    "pitchbook.com", "opencorporates.com", "glassdoor.com", "indeed.com",
    "jobstreet.com", "listedcompany.com", "tradingview.com", "marketscreener.com",
...
class SearchSource:
    ...
    def _live(
        ...
        """검색 공급자로 기업 도메인 후보를 수집한다(현지화 + 동의어 다중 쿼리 **병렬** + dedup + 캡).

        업종을 영어 동의어 목록으로 옮겨(한글 업종어 헛방 수정) 동의어마다 별도 쿼리를 **병렬로
        동시에** 던지고(순차 대기 제거 — 산출 최대화), 결과 합집합을 도메인 dedup·blocklist·
        ``seen``(글로벌 정규화 도메인) 필터를 거쳐 cap 까지 모은다. 각 쿼리는 자기 페이징과
        신규비율 조기중단(``search_min_new_ratio``)을 독립 수행한다(중복에 과금 방지, 제약 ①).
        """
        s = self._settings
        ...
        def _run(query: str) -> list[DiscoveredCompany]:
            return self._fetch_query(
                provider, segment, query, gl=gl, lr=lr, cap=cap, seen_global=seen_global
            )
        ...
        if len(queries) > 1:
            ...
            results = [_run(queries[0])] if queries else []

        # 쿼리 간 합집합 — 도메인 기준 dedup 후 cap 까지 모은다(쿼리 입력 순서 보존).
        out: list[DiscoveredCompany] = []
        merged: set[str] = set()
        for cand_list in results:
            for dc in cand_list:
                if dc.domain in merged:
                    continue
                merged.add(dc.domain)
                out.append(dc)
                if len(out) >= cap:
                    break
            if len(out) >= cap:
                break
        log.info("search.live", segment=segment.label, n=len(out), queries=len(queries))
        return out
    ...
    def _fetch_query(
        ...
        *,
        ...
        lr: str,
        cap: int,
        seen_global: frozenset[str] | set[str],
    ) -> list[DiscoveredCompany]:
        """한 동의어 쿼리의 페이징 루프 — 페이지 dedup + seen_global 필터 + 신규비율 조기중단.

        스레드별로 독립 실행된다(자기 ``seen_local``·결과 리스트만 만지고 공유 가변상태 없음).
        ``seen_global`` 은 읽기 전용. 호출부가 쿼리 간 합집합 dedup·최종 cap 을 담당한다.
        """
        min_new_ratio = self._settings.search_min_new_ratio
        out: list[DiscoveredCompany] = []
        seen_local: set[str] = set()  # 이 쿼리의 페이지 간 도메인 중복 제거.
        start = 1
        while len(out) < cap and start <= provider.max_start:
            ...
            # 신규비율 조기중단: 이 쿼리의 한 페이지 신규 비율이 임계 미만이면 더 사지 않는다.
    ...
    def _candidate(
        ...
        # 포함 — 기업 도메인에 이 라벨이 단독으로 오는 경우는 드물어 오차단 위험 낮다.
        if any(p in {"gov", "edu", "ac", "mil", "go", "or", "ne", "gob"} for p in domain.split(".")):
            return None
        seen.add(domain)
