...
from .fill import _DOMAIN_OVERSHARE_CAP, _StallWatchdog, _close_own, _dc_from_row, _scoped
...
@dataclass
class PromoteRun:
    """런(세대) 범위 공유 컴포넌트 — 배치 간 재사용한다.

    ``classifier`` 의 ``industry_llm_max_calls`` 는 **런당** 상한(config)이라 배치마다 새로
    만들면 상한이 배치마다 리셋돼 과금 보호가 무력화된다(리뷰 HIGH). ``registry_checker`` 는
    httpx 를 지연 보유하므로 런 종료 시 :meth:`close` 로 1회 정리한다. 호출자(스크립트·
    ``segment-run``)가 :meth:`open` 으로 만들고 ``try/finally`` 로 닫는다.
    """
