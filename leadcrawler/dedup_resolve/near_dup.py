...
# 이름 시작·끝에 붙은 것만(중간 출현은 상호 일부일 수 있어 보존 — Codex 설계안 앵커 제안).
_ATTACHED_LEGAL = re.compile(
    r"^\s*(주식회사|유한회사|유한책임회사|합자회사|합명회사|재단법인|사단법인)"
    r"|(주식회사|유한회사|유한책임회사|합자회사|합명회사|재단법인|사단법인)\s*$"
)
