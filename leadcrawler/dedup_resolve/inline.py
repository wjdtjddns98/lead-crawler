...
def find_inline_duplicate(
    ...
    설계 메모:
    - **생존자 선정은 positional(기존 우선)** — 신규를 항상 기존 생존자로 흡수한다. 신규가
      더 권위 높아도(reg:) 여기선 역방향 링크가 될 수 있으나, 추출은 이미 스킵됐고 식별
      컬럼은 보존되며 가역(audit)이라 손실 없음. 최종 골든레코드 권위 재선정은 C3 가 담당.
    - ``ilike('%root%')`` 는 leading-% 라 인덱스를 못 타지만(대형 원장에서 비용), 교차key
      재발견에서만 발동(드묾)하고 ``MAX_BLOCK_SIZE`` 캡이 하류 O(n²)를 막는다. 볼륨이 커지면
      정규화 도메인 인덱스 컬럼으로 교체 권장(별도 마이그레이션).
    - 동시 워커에선 얕은 ``duplicate_of`` 체인이 생길 수 있다(사이클은 불가) — C3 경로압축 위임.
    """
