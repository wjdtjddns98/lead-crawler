...
class Settings(BaseSettings):
    ...
    # KR 발견 화이트리스트 = NPS(국민연금) + 네이버 지역검색. 구 'NPS 단독'(PO 2026-07-13)에서
    # 지역검색을 추가(2026-08-19 — 업체 DB 기반이라 헤드라인 오탐 없음, 1회성 KR 크롤 3,073
    # 승격 실증). DART 는 NPS 와 중복(캐시 조인·relink 로 부착), SERP 검색은 뉴스/블로그 제목
    # 오탐 실측(2026-07-13, <b>태그 헤드라인 57건)으로 계속 차단. False=기존 다소스 발견.
    ...
    # 발견(discovery) 세그먼트 동시성 — 세그먼트들을 동시에 발견(네트워크 수집)한다. 1(기본)
    # 이면 현 동작(단일 스레드 순차). 등록처(DART/EDGAR)는 세그먼트당 수천 HTTP 호출로 길게
    # 블로킹돼 처리량 병목이라, 올리면 처리량↑. 단 같은 호스트를 동시에 때리면 429 위험이 커지므로
