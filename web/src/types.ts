...
// 못 붙으면 후자(토큰 미설정이 기본값이라 그때는 전량 후자). gbiz_jp(BE #458)도 같은 규약이나
// name_en 보유율이 2.5%(BE 실측)라 절대다수가 후자다. 일본 기업은 일문이 두 슬롯 중 하나엔
// 있지만 영문은 없을 수 있다 — 다만 gbiz_jp 는 도메인 있는 행만 들어와 홈페이지로는 잡힌다.
