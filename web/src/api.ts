...
// 몫(total 동봉). name_eng 의 표기는 **국가**로 갈린다(BE #478, 2026-09-04 PO 지시) — 한국(KR)
// 기업은 표시명 name 이 국문·name_eng 가 **영문명**(DART·NPS), 그 외 전 국가는 표시명이
// **영문 상호**·name_eng 가 **원어 원문**(GLEIF 영문 법인명 또는 홈페이지에 적힌 영문 상호를
// 적재 시점에 추출. 둘 다 없으면 표시명 원어 유지·name_eng 없음). 원래 일본 소스(BE #412
// EDINET·#445 jp_assoc)에만 적용되던 슬롯 규약이 전 국가로 확대된 것이라, 아래 일본 서술은
// 그 규약의 선례이자 "영문이 없는 행" 의 실측이다. 소급은 자동이 아니다(BE CLI
// backfill-name-eng 를 수동 실행) — 돌리기 전까지 기존 행은 원어 표시명, 신규 행은 영문
// 표시명으로 **섞여 보인다**. 그래서 "영문명"이라고 안내하면 원어 검색이 되는 사실이 가려진다.
...
// 금융청 등록·면허 일람(BE #449 fsa_jp — 은행 141·신금 254·증권·자산운용 1,331, 2026-08-31
// BE 실측)은 그 예외를 **소수에서 다수로** 바꾼다: gBizINFO 토큰이 있어야 영문 상호(name_en)가
// 붙어 EDINET 슬롯이 되고, 토큰 없이 들어온 분은 전량 표시명이 일문·name_eng 없음이다.
// 토큰이 붙어도 gBizINFO 가 영문명을 안 주거나 비라틴이면 그 행만 일문으로 남아 **같은 소스
// 안에 일문·영문이 섞인다** — JSDA 처럼 소스 전체에 영문이 부재한 것과 달리 행 단위라, 어느
// 표기로 들어오는지를 화면에서 가정할 수 없다.
// 일본 전수 발견 소스(BE #458 gbiz_jp)는 규모가 다르다 — 슬롯 규약은 fsa_jp 와 같은데
// gBizINFO 상세의 name_en 보유율이 **2.5%**(BE #458 실측)라, 토큰이 정상이어도 절대다수가
// 표시명 일문·name_eng 없음으로 들어온다. fsa_jp 의 행 단위 편차와 달리 데이터에 영문이
// 없는 쪽이라 시간이 지나도 안 풀린다. 대신 이 소스는 **도메인이 있는 행만** 열거하므로
// 홈페이지 질의로는 항상 잡힌다 — 영문 질의가 막힐 때의 실질적 우회로다.
// → 정리하면 KR 외 기업은 원어가 name·name_eng 중 어딘가엔 항상 있어 **원어 질의로는 늘
// 잡히지만** 영문은 아예 없을 수 있다(#478 이후에도 근거 없는 행은 원어 유지 — 추측 금지).
// 안내 문구는 원어 질의를 권하는 쪽으로 쓴다.
