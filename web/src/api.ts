...
// gBizINFO 전수 소스(BE #458 gbiz_jp)는 슬롯 규약이 같은데 **비율이 정반대**다: 상세의
// name_en 보유율이 2.5%(2026-09-01 BE 실측)라 절대다수가 일문·name_eng 없음으로 남는다 —
// JSDA 처럼 데이터 부재형이라 영문 질의로는 영영 안 잡힌다. 대신 이 소스는 도메인 있는 행만
// 원장에 넣으므로(company_url 공란 67% 제외) **홈페이지 질의로는 항상 잡힌다** — 영문이
// 막힐 때의 우회로. 참고로 gBizINFO 호출은 BE #456 에서 API 호스트가 교정됐다(그전 조회
// 실패분은 fsa_jp 라도 일문으로 들어왔다).
// → 정리하면 일본 기업은 일문이 name·name_eng 중 어딘가엔 항상 있어 **일문 질의로는 늘
// 잡히지만** 영문은 아예 없을 수 있다. 안내 문구는 일문 질의를 권하고, 영문으로 못 찾을 땐
// 홈페이지 URL 질의를 대안으로 제시하는 쪽으로 쓴다.
