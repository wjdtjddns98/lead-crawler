...
// 금융청 등록·면허 일람(BE #449 fsa_jp, 최대 1,689사)은 그 예외를 **소수에서 다수로** 바꾼다:
// gBizINFO 토큰(LEADCRAWLER_GBIZINFO_API_TOKEN)이 있어야 영문 상호가 붙어 EDINET 슬롯이
// 되는데 토큰이 발급 전이라, 그전 유입분은 전량 표시명이 일문·name_eng 없음이다. 토큰이
// 붙어도 커서가 지나간 행은 재조회하지 않아 **같은 소스 안에 일문·영문이 섞인다**(JSDA 처럼
// 데이터에 영문이 없어서가 아니라 발견 시점 탓 — 원인이 다르다).
