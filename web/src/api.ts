...
// 뒤이어 붙은 일본 소스 둘이 이 예외를 **소수에서 다수로** 바꾼다. 금융청 등록·면허 일람
// (BE #449 fsa_jp, 최대 1,689사)은 gBizINFO 로 영문 상호를 붙여 EDINET 슬롯이 되지만, API
// 호스트가 틀려 있던 BE #456 이전 유입분은 전량 표시명이 일문·name_eng 없음이다. 커서가
// 지나간 행은 재조회하지 않아 **같은 소스 안에 일문·영문이 섞인다**(발견 시점 탓).
// 일본 전수 발견 소스(BE #458 gbiz_jp)는 원인이 다르다 — 슬롯 규약은 같은데 gBizINFO 상세의
// name_en 보유율이 **2.5%** 라, 토큰이 정상인 지금도 절대다수가 표시명 일문·name_eng 없음이고
// 시간이 지나도 안 풀린다(JSDA 와 같은 데이터 부재형). 대신 이 소스는 도메인이 있는 행만
// 열거해 **홈페이지 질의로는 항상 잡힌다** — 영문 질의가 막힐 때의 실질적 우회로다.
