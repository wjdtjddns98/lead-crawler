...
- 경계: 워크벤치 UI 트랙(예: PLAN C4)은 **백엔드 API만** 우리가, React 컴포넌트는 프론트 담당. FastAPI는 백엔드.
...
- 소유권은 `.github/CODEOWNERS` 가 자동 리뷰 라우팅(`web/**`→프론트, 그 외→백엔드).
- CI 는 영역 분리: `backend.yml`(파이썬, `web/**` 제외 시 실행) / `web.yml`(`web/**` 변경 시 실행).
  → 백엔드 PR 은 프론트 빌드를, 프론트 PR 은 백엔드 테스트를 **서로 안 기다린다**.
- **FE↔BE 계약(엔드포인트·요청/응답 스키마) 변경은 PR 본문에 명시 + 상대에게 공유**(양쪽 영향).
- 권장: main 에 브랜치 보호(PR 필수 + CI green 필수) 설정. 현재 미설정(직접 push 가능 — 규칙으로만 금지).
