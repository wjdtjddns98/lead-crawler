...
- 커밋 전 로컬 게이트 green 필수 — **백엔드**: `uv run ruff check .` + `uv run pytest -q`(의존성은 `uv sync`) / **프론트**: `npm run build`(tsc+vite)(+lint).
...
- 출력은 고정 엑셀 12컬럼 서식: J·K=대문자 O/X, E=문의폼 URL(폼 있으면 클릭 이동, 없으면 "X"), 이메일 없고 폼만 있으면 J="사이트 내 문의폼".

## 4. 백엔드 — 운영 *(백엔드 전용)*
- 예산: 월 운영비 ~50만원 한도(`cost_ledger` 추적).
- Notion 운영 서식(일일보고·스크럼·현황)은 **이 리포 밖에서 작성**한다 — 리포 안의 자동
  리포터(`integrations/notion.py`·`reporting.py`·`scheduler/`)는 제거됐다. Nutti 허브 단위
  단일 스케줄로 일원화. 이 리포에 Notion 연동을 다시 추가하지 않는다.
...
## 7. 리뷰·설계 이중화 — Codex 교차검증 *(백엔드 전용, 2026-07-13)*
운영구조: **Claude Code(메인)** = 기획·작업분해·코드구현·테스트 실행 → **Codex** = 독립
코드리뷰·버그탐색·수정안 제안(파일 직접 수정 금지 — 적용은 메인이).
- **리뷰(§0 의 리뷰어 검증을 이것으로 수행)**: `codex-reviewer` 에이전트(`.claude/agents/`)와
  Claude 리뷰어(code-reviewer 계열)를 **병렬**로 돌리고, 두 결과를 **메인 세션(Opus 4.8)이
  종합판단**(중복 병합·심각도 조정·채택/기각)한다.
- **설계(신기능·중형 이상)**: Codex 와 Claude 가 **각자 독립 설계안**을 만들고(서로의 안을
